...
function initObservers() {
  ...
  for (let i = 0; i < obs_num; i++) {
    ...
    var obs_nights_obs = sheet_history.getSheetValues(4, i + 1, obs_num_nights_obs + obs_num_nights_missed, 1); // Get list of all nights observed (or not)
...
function parseFollowup(obs_names) {
  ...
  if (obs_names[0].includes(latest_entry_name)) {                             // If existing observer
    ...
    var obs;
    if (sheet_followup.getSheetValues(2, 5, 1, 1)[0][0] == "Yes") {           // If person observed, update values and add to observer list 
