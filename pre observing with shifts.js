...
const DAY_COL_NUM = 4;
const EXP_COL_NUM = 5;
const SHIFT_COL_NUM = 6;
const OPT_COL_NUM = 7;

// Column numbers for parsed sheet
const PARSED_DAY_COL_NUM = 1;
const PARSED_SHIFT_COL_NUM = 2;
const NUM_OBS_COL = 3;
const SENIOR_COL_NUM = 4;
const JUNIOR_COL_NUM = 5;
...
// Class for days
class ScheduledDay {
  constructor(day, num_obs, obs_list, shift) {
    this.day = day;
...
function initDays() {
  ...
  var shifts = sheet_schedule.getSheetValues(2, PARSED_SHIFT_COL_NUM, days_num, 1);

  for (let i = 0; i < days_num; i++) {
    var curr_day = days[i][0];
    var curr_num_obs = num_obs[i][0];

    var curr_shift = shifts[i][0];
    curr_shift = curr_shift.toString().substring(0, 3);
    if (curr_shift === "Fir") {
      curr_shift = "First";
    }
    else {
    ...
    var day = new ScheduledDay(curr_day, curr_num_obs, curr_obs_list, curr_shift);
    days_list.push(day);
  }
  parseSignup(days, shifts);
};


function parseSignup(days, shifts) {
  sortBySubmission();
  ...
  var latest_entry_shift = sheet_signup.getSheetValues(2, SHIFT_COL_NUM, 1, 1)[0][0];
  ...
  var string_entry_shift = latest_entry_shift.toString().substring(0, 3);

  var name_color = 'orange';
  var shift_color = 'orange';

  if (string_entry_shift === "Fir") {
    string_entry_shift = "First";
  }
  else {
    string_entry_shift = "Second";
    shift_color = "blue";
  }

  let email_date = latest_night_obs.toString().substring(0, 15) + ", Shift: " + string_entry_shift;

  if (latest_entry_exp === "Senior") { 
    addSenior(latest_entry_name, latest_night_obs, latest_entry_email, days, email_date, shifts, string_entry_shift, shift_color);
    return;
  }

  var arr = checkDuplicateDays(days, shifts, latest_night_obs, string_entry_shift);
  if (arr[0]) {                                                                 // If day with same shift is already on schedule, don't re-add. Only add name
    ...
    sheet_schedule.getRange(index, index2, 1, 1).setValue(latest_entry_name).setFontColor(name_color);   
    ...
    sheet_schedule.getRange(index, JUNIOR_COL_NUM, 1, 1).setValue(latest_entry_name).setFontColor(name_color);          
    // Set first observer to entry name
    sheet_schedule.getRange(index, PARSED_SHIFT_COL_NUM, 1, 1).setValue(string_entry_shift).setFontColor(shift_color); 
    // Set shift and shift color
...
// Check to make sure same day isn't being re-added
function checkDuplicateDays(days, shifts, day_name, shift) {
  for (let i = 0; i < days.length; i++) {
    var curr_day = days[i][0].toString();
    var curr_shift = shifts[i][0];
    if ((curr_day.localeCompare(day_name) === 0) && (curr_shift.localeCompare(shift) === 0)) {
      var arr = [true, i];
      return arr;
    }
  }
...
function addSenior(latest_entry_name, latest_night_obs, latest_entry_email, days, email_date, shifts, shift, shift_color) {
  var arr = checkDuplicateDays(days, shifts, latest_night_obs, shift);
  if (arr[0]) {                                                                 // If day is already on schedule, don't re-add. Only add name
    let curr_day = days_list[arr[1]];
    let index = days_list.indexOf(curr_day) + 2;                                // Row to list new observer

    if (!sheet_schedule.getRange(index, SENIOR_COL_NUM, 1, 1).isBlank()) {      // If a senior observer has not already signed up, do not add.
      sendFollowupEmail(latest_entry_name, email_date, latest_entry_email, false);
      return;
    }
    ...
    sheet_schedule.getRange(index, SENIOR_COL_NUM, 1, 1).setValue(latest_entry_name).setFontColor('blue');  
    sheet_schedule.getRange(index, PARSED_SHIFT_COL_NUM, 1, 1).setValue(shift).setFontColor(shift_color); 
    // Set senior observer
